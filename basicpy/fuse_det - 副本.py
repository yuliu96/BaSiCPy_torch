...
class FUSE_det:
            ...
            if (not os.path.exists(
                ...
                trans_path = os.path.join(
                    ...
                    "{}".format(illu_name if not det_only_flag else self.sample_params["topillu_dorsaldet_data_saving_name"])
                     + "{}_coarse_reg.tif".format("" if (not self.train_params["destripe_preceded"]) else "_destripe"),
                )
    ...
    def apply(self,
        ...
        if (not skip_apply_registration) or (not os.path.exists(trans_path)):
            ...
            f_flag = translating_information["f_flag"]
            flip_axes = translating_information["flip_axes"] if f_flag else None
        ...
        if (not skip_refine_registration) or (not os.path.exists(trans_path)):
            ...
            source_pcd = o3d.geometry.PointCloud()
            source_pcd.points = o3d.utility.Vector3dVector(
                source_points[::z_upsample_ratio*(xy_upsample_ratio**2), :]
                * np.array(
                    [
                        z_spacing,
                        xy_spacing,
            ...
            target_pcd = o3d.geometry.PointCloud()
            target_pcd.points = o3d.utility.Vector3dVector(
                target_points[::z_upsample_ratio*(xy_upsample_ratio**2), :]
                * np.array(
                    [
                        z_spacing,
                        xy_spacing,
            ...
            # aaa = np.load(os.path.join(
            #         save_path,
            #         "high_res",
            #         "reginfo_refine.npy",
            #     ), allow_pickle=True).item()["transformation"]
...
def volumeTranslate_compose(
    ...
    commonData = np.zeros((int(max(padding_z, zmax)),
