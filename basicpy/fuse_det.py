...
def define_translating_information(
    AffineTransform_float_3_3_inverse: np.ndarray = None,
    fixed_inverse: np.ndarray = None,
    AffineMapZXY: list = [0, 0, 0],
    T2: np.ndarray = None,
    padding_z: int = None,
    T_flag: int = 0,
    flip_axes: tuple = tuple([]),
):
    kwargs = locals()
    return kwargs
...
class FUSE_det:
    ...
    def train(
        ...
        if not det_only_flag:
            if save_separate_results:
                reconVol, reconVol_separate, boundary_mask = fusionResultFour(
                    boundaryTop,
                    boundaryBottom,
                    boundaryEFront,
                    boundaryEBack,
                ...
                reconVol, boundary_mask = fusionResultFour(
                    boundaryTop,
                    boundaryBottom,
                    boundaryEFront,
                    boundaryEBack,
        ...
        if T_flag:
            ...
            boundary_mask = boundary_mask.swapaxes(1, 2)
            if save_separate_results:
    ...
    def apply(
        ...
        boundary: np.ndarray,
        translating_information: str = None,
        z_upsample_ratio: int = None,
        xy_upsample_ratio: int = None,
        z_spacing: int = None,
        xy_spacing: int = None,
        ...
        if translating_information != None:
            translating_information = define_translating_information(
                **translating_information
            )
        if not os.path.exists(save_path):
            print("saving path does not exist.")
            return
        ...
        if require_registration:
            if (z_spacing == None) or (xy_spacing == None):
                ...
                return
        if translating_information != None:
            if (z_upsample_ratio == None) or (xy_upsample_ratio == None):
                print("upsample ratios are missing.")
                return
        ...
        final_dorsal_path = os.path.join(data_path, sample_name, dorsal_det_data_path)
        ...
        if translating_information != None:
            if (not skip_apply_registration) or (not os.path.exists(trans_path)):
                ...
                if translating_information["padding_z"] is not None:
            ...
            final_dorsal_path = os.path.join(
                save_path,
                "high_res",
        ...
        if require_registration:
            if (not skip_refine_registration) or (not os.path.exists(trans_path)):
                ...
                target_points = DoG(
                    ventral_det_data, device=self.train_params["device"]
                )
        ...
        dorsal_det_data_handle = h5py.File(final_dorsal_path, "r")
        dorsal_det_data = dorsal_det_data_handle["X"]
        z, x, y = dorsal_det_data.shape

        boundary = boundary.astype(bool)
        tmp0, tmp1 = np.arange(0, x, 10), np.arange(10, x + 10, 10)
        boundary_ze = np.zeros((z, boundary.shape[1], boundary.shape[2]), bool)
        if boundary.shape[0] < z:
            for p, q in zip(tmp0, tmp1):
                boundary_ze[:, p:q, :] = (
                    F.interpolate(
                        torch.from_numpy(boundary[:, p:q, :].transpose(1, 0, 2)[None])
                        .to(self.train_params["device"])
                        .to(torch.float),
                        size=(z, boundary.shape[2]),
                    )
                    .cpu()
                    .data.numpy()[0]
                    .transpose(1, 0, 2)
            ...
            boundary_ze = boundary[:z, :, :]

        # boundary = z_upsample_ratio * boundary
...
def fusionResult(
    ...
    if boundary.ndim == 2:
        ...
        boundary = boundary.astype(bool)[None]
    ...
    for ii in tqdm.tqdm(
        ...
        if bottomMask.shape[-2:] != (m, n):
            bottomMask = F.interpolate(
                bottomMask,
                size=(m, n),
            )
            topMask = F.interpolate(
                topMask,
                size=(m, n),
            )
...
def fusionResultFour(
    ...
    del illu_front, illu_back
    if save_separate_results:
        return reconVol, reconVol_separate, boundary_mask
    else:
        return reconVol, boundary_mask
...
def volumeTranslate_compose(
    ...
    if z > zback:
        z_list = np.concatenate((np.ones(z - zback, dtype=np.int32) * -1, z_list))
